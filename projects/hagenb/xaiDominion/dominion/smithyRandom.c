...
int main() 
{
	...
	for (i = 0; i < (num_tests); i++)
	{
		...
		int initialHandCount = testG.handCount[thisPlayer]+1;

		//call cardEffect, so that this will work whether function is separated out or not
		cardEffect(smithy, choice1, choice2, choice3, &testG, handPos, &bonus);

		//cards drawn calculated by final count + 1 (for discarded card) - initial count + 1
		//printf("\tHandCount for this player: %d\n", testG.handCount[thisPlayer]+1);
		printf("\tCards drawn = %d, expected = %d\t", testG.handCount[thisPlayer]+2 - initialHandCount, 3);
		pass(testG.handCount[thisPlayer]+2 - initialHandCount, 3);
